...
import { PATH, mockBannedUser, mockUser, mockUsers, routeChildren } from "./fixtures";
...
describe("VRT user page - ban dialog - active user", async () => {
  beforeEach(() => {
  ...
  it.each(textMatrix)("$name", async ({ theme, device }) => {
    ...
    const { container } = await render(<Stub initialEntries={[PATH]} />, {
      wrapper: withNuqsTestingAdapter({
        searchParams: { user_id: mockUser.id, dialog: "ban" },
      }),
    });
...
describe("VRT user page - ban dialog - banned user", async () => {
  beforeEach(() => {
    vi.mocked(auth.admin.listUsers).mockResolvedValue({
      data: { users: mockUsers, total: mockUsers.length },
      error: null,
    });
    vi.mocked(auth.admin.getUser).mockResolvedValue({ data: mockBannedUser, error: null });
  });
  ...
  it.each(textMatrix)("$name", async ({ theme, device }) => {
    ...
    const { container } = await render(<Stub initialEntries={[PATH]} />, {
      wrapper: withNuqsTestingAdapter({
        searchParams: { user_id: mockBannedUser.id, dialog: "ban" },
      }),
    });
