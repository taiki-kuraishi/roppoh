...
import { PATH, mockSessions, mockUser, mockUsers, routeChildren } from "./fixtures";
...
describe("VRT user page - sessions dialog", async () => {
  beforeEach(() => {
    vi.mocked(auth.admin.listUsers).mockResolvedValue({
      data: { users: mockUsers, total: mockUsers.length },
      error: null,
    });
    vi.mocked(auth.admin.listUserSessions).mockResolvedValue({
  ...
  it.each(textMatrix)("$name", async ({ theme, device }) => {
    ...
    const { container } = await render(<Stub initialEntries={[PATH]} />, {
      wrapper: withNuqsTestingAdapter({
        searchParams: { user_id: mockUser.id, dialog: "sessions" },
      }),
    });
