...
type Params = Parameters<NonNullable<(typeof auth)["admin"]["setRole"]>>[0];
...
export const useSetRoleMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.setRole(params);
