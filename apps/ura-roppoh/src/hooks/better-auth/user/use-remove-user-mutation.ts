...
type Params = Parameters<NonNullable<(typeof auth)["admin"]["removeUser"]>>[0];
...
export const useRemoveUserMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.removeUser(params);
