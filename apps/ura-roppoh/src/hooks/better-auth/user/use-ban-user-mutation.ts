...
type Params = Parameters<NonNullable<(typeof auth)["admin"]["banUser"]>>[0];
...
export const useBanUserMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.banUser(params);
