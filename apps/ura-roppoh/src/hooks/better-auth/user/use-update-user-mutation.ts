...
type Params = Parameters<NonNullable<(typeof auth)["admin"]["updateUser"]>>[0];
...
export const useUpdateUserMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.updateUser(params);
