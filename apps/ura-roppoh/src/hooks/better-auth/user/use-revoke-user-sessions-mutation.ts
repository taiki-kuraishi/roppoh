...
import { USE_USER_SESSIONS_KEY } from "./use-user-sessions";

type Params = Parameters<NonNullable<(typeof auth)["admin"]["revokeUserSessions"]>>[0];
...
export const useRevokeUserSessionsMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.revokeUserSessions(params);
  ...
  return useMutation({
    ...
    onSuccess: async (_data, variables) => {
      await query.invalidateQueries({ queryKey: [USE_USER_SESSIONS_KEY] });
