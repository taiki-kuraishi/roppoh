...
type Params = Parameters<NonNullable<(typeof auth)["admin"]["createUser"]>>[0];
...
export const useCreateUserMutation = (args: Args) => {
  const query = useQueryClient();

  const mutationFn = async (params: Params) => {
    const { data, error } = await auth.admin.createUser(params);
