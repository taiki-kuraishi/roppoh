...
testWithMswMock(handlers)("vrt /oidc-client dark", async ({ page }) => {
  await setAuthenticated(page);
  await setTheme({ page, theme: "dark" });
  await page.goto("/oidc-client", { waitUntil: "networkidle" });
  await expect(page).toHaveScreenshot({ fullPage: true });
});

testWithMswMock(handlers)("vrt /oidc-client light", async ({ page }) => {
  await setAuthenticated(page);
