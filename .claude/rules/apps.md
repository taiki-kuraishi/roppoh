...
| --------------- | ------------------------------- | --------------------------------- | ------------------------------------------- |
| `neo-fujimatsu` | Vite + Hono/Inertia + CF Worker | `src/client/` + `src/server/`     | `apps/neo-fujimatsu/directory-structure.md` |
| `web-console`   | Vite + Inertia + CF Worker      | `app/`(Inertia, PascalCase pages) | `apps/web-console/directory-structure.md`   |
| `roppoh`        | Vite + Inertia + CF Worker      | `app/`(Inertia, PascalCase pages) | `apps/roppoh/directory-structure.md`        |
| `emdash`        | Astro + CF Worker               | `src/`(Astro ファイルベース)      | `apps/emdash/directory-structure.md`        |
...
- そのアプリの `directory-structure.md` を `.claude/rules/apps/<app>/` に追加する
