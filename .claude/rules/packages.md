...
| Package              | 役割                                                     | 個別ルール                         |
| -------------------- | -------------------------------------------------------- | ---------------------------------- |
| `better-auth`        | better-auth サーバ設定 + DB schema                       | `better-auth/better-auth.md`       |
| `better-auth-query`  | better-auth 用 TanStack Query フック                     | —                                  |
| `oidc-client`        | 公開 SPA 用 OIDC クライアント(react-oidc-context ラッパ) | —                                  |
| `shadcn`             | shadcn/ui 共有 UI(生成コード)                            | `shadcn/shadcn.md`                 |
| `grafana-dashboards` | TS→Grafana JSON 生成器                                   | → `k8s.md`                         |
| `oxlint-plugins`     | カスタム oxlint ルール                                   | `oxlint-plugins/oxlint-plugins.md` |
